...
const Reviews = () => {
  ...
  const { data: reviews, isLoading } = useQuery("reviews", () =>
    fetch(`https://techfly-server-side.vercel.app/review`).then((res) =>
      res.json()
    )
  );
  ...
  return (
    <div className="px-20">
      <p className="text-2xl text-center font-semibold mt-20 mb-12 text-secondary uppercase ">
        Reviews
      </p>
          ...
          .map((review) => (
            <div key={review._id}>
              <div className="border-2 border-yellow-400 rounded-lg px-4 py-6 flex flex-col justify-center items-center gap-6 mr-5 ">
                <img
                  src={image ? image : proofile}
                  alt=""
                  className="h-[80px] rounded-full w-[80px] object-cover mb-2"
                />
