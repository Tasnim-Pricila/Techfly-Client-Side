...
const Parts = () => {
  ...
  const { data: parts, isLoading } = useQuery("parts", () =>
    fetch(`https://techfly-server-side.vercel.app/parts`).then((res) =>
      res.json()
    )
  );
  if (isLoading) {
  ...
  return (
    <>
      <p
        className="text-3xl text-center font-semibold mb-12 mt-20 uppercase text-secondary"
        id="parts"
      >
        Parts
      </p>
      <div className="md:px-24 px-4 mb-12 grid md:grid-cols-3 grid-cols-1 gap-8">
        {parts.data
          ...
          .map((part) => (
            <div key={part._id}>
              <div
                className="card card-compact shadow-xl border px-4 py-6"
                data-aos="zoom-in-up"
                ...
                <div className="card-body md:h-[400px]">
                  ...
                  <div className="card-actions justify-end">
                    <button
                      ...
                      onClick={() => handlePurchase(part._id)}
                    >
                      Purchase
                    </button>
                  </div>
      ...
      <div className="flex justify-center mb-8">
        <button className="btn btn-secondary btn-outline" onClick={handleClick}>
          All Parts
        </button>
      </div>
    </>
