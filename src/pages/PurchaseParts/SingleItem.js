...
const SingleItem = ({ part, setModalData, minQuantity, refetch }) => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch("https://techfly-server-side.vercel.app/purchase", {
      method: "POST",
      headers: {
    ...
    <div>
      <div className="">
        <div className="card lg:card-side shadow-xl md:mx-20 mt-20 md:px-20">
          <figure>
            <img src={image} alt="Album" className="w-52" />
          </figure>
          <div className="card-body">
            ...
            <p className="pb-8">
              {" "}
              <b>Minimun Order Quantity:</b> {minQty}
            </p>
      ...
      <div className="md:w-1/3 mx-auto border-4 my-8">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 my-8 ">
          <label htmlFor="Quantity" className="font-semibold md:pl-24 pl-8">
            Quantity
          </label>
          <input
            ...
            type="number"
            defaultValue={minQty}
            className="input input-bordered border-black w-full max-w-xs self-center focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:border-transparent"
            onKeyUp={handleQuantity}
            required
          />
          <small className="text-center text-error font-bold">{error}</small>
          ...
          <input
            type="submit"
            className="btn btn-warning w-[320px] self-center"
            value="Purchase"
            disabled={quantity < minQty || quantity > availableQty}
          />
        </form>
      </div>
    </div>
