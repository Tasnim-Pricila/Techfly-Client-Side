...
const MyProfile = () => {
  ...
  const {
    ...
    fetch(`https://techfly-server-side.vercel.app/user/${email}`, {
      method: "GET",
      headers: {
    ...
    <div className="lg:w-2/3 border-4 border-purple-400 py-6 px-8 rounded-lg mb-12 mr-4 lg:mr-0">
      <div className="flex justify-between border-b-2 border-slate-700 pb-2 items-center mb-6">
        ...
        <div className="flex items-centerbasis-1/2 justify-end">
          <button
            className="flex btn btn-primary btn-outline group pt-1 btn-sm"
            onClick={handleEdit}
          >
            Edit
            <FontAwesomeIcon
              icon={faEdit}
              className="text-primary pl-4 group-hover:text-white"
            ></FontAwesomeIcon>
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 justify-center items-center">
        ...
        {userInfo?.data?.phone && (
          <p>
            {" "}
            <b> Phone: </b> {userInfo.data.phone}
          </p>
        )}
        {userInfo?.data?.city && (
          <p>
            {" "}
            <b> City: </b> {userInfo.data.city}
          </p>
        )}
        {userInfo?.data?.district && (
          <p>
            {" "}
            <b> District: </b> {userInfo.data.district}
          </p>
        )}
        {userInfo?.data?.address && (
          <p>
            {" "}
            <b> Address: </b> {userInfo.data.address}
          </p>
        )}
      ...
      <div className="mb-8 flex flex-col items-center">
        {userInfo?.data?.education ? (
          ...
          <button
            className="text-xl font-bold text-primary mb-8 mt-8 border-slate-700 border px-4 py-2 rounded"
            ...
            {userInfo?.data?.phone ? (
              ...
              <>
                <input
                  placeholder="Phone"
                  type="text"
                  className="input input-bordered input-info w-full max-w-xs font-semibold self-center"
